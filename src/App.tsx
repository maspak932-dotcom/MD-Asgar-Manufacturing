  ...
  return (
    <div className="min-h-screen bg-[#f5f1ea] text-slate-900 overflow-x-hidden font-sans">
      ...
      <header className="bg-black">
        {/* NAYA TOP BANNER YAHAN LAGA HAI */}
        ...
        <div className="text-center px-5 py-16 md:py-20 bg-black">
          ...
          <div className="flex items-center justify-center gap-4 mt-10 flex-wrap">
            <a href={whatsappLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-3 bg-yellow-500 hover:bg-yellow-400 text-black px-8 py-4 rounded-2xl font-bold text-base md:text-lg transition-all">
              Contact On WhatsApp
              <ChevronRight size={20} />
            </a>
            <a href="#collection" className="inline-flex items-center gap-2 border border-zinc-700 hover:border-yellow-400 text-white px-8 py-4 rounded-2xl font-semibold text-base md:text-lg transition-all">
              View Collection
            </a>
          </div>
        </div>
      ...
      <main id="collection" className="max-w-7xl mx-auto px-5 md:px-8 py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {products.map((product) => (
            <div key={product.id} className="bg-white rounded-[2rem] overflow-hidden shadow-xl hover:-translate-y-2 hover:shadow-2xl transition-all duration-500">
              <div className="bg-white h-[300px] md:h-[350px] p-4 flex items-center justify-center overflow-hidden">
                <img src={product.image} alt={product.name} className="w-full h-full object-contain" />
              </div>
              <div className="p-7 md:p-8">
                <span className="text-yellow-500 text-xs font-mono tracking-[2px]">{product.index}</span>
                <h4 className="text-xl md:text-2xl font-bold mt-2 mb-3">{product.name}</h4>
                <p className="text-gray-600 leading-relaxed mb-8">{product.description}</p>
                <a href={whatsappLink} target="_blank" rel="noreferrer" className="w-full bg-black hover:bg-yellow-500 hover:text-black text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all">
      ...
      {/* NAYA MIDDLE SHOWCASE BANNER YAHAN LAGA HAI (Footer se theek pehle) */}
      <section className="w-full mt-10">
        <img 
          src="/IMG_20260723_145155_841.png" 
          alt="Premium Trolley Bag Showcase" 
          className="w-full max-w-7xl mx-auto h-auto object-contain block"
        />
      </section>

      {/* FOOTER */}
      <footer id="contact" className="bg-black text-white py-20 px-5 md:px-8">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-16">
          <div>
            ...
            <div className="space-y-5">
              <a href={`tel:${contactNumber}`} className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 p-5 rounded-2xl hover:border-yellow-500/50 transition-colors">
                <div className="bg-yellow-500/20 p-3 rounded-xl">
                  <Phone className="text-yellow-400" size={22} />
                </div>
                <span className="text-lg font-medium">{contactNumber}</span>
              </a>
              <a href={`mailto:${contactEmail}`} className="flex items-center gap-4 bg-zinc-900 border border-zinc-800 p-5 rounded-2xl hover:border-yellow-500/50 transition-colors">
                <div className="bg-yellow-500/20 p-3 rounded-xl">
      ...
      {/* WHATSAPP BUTTON */}
      <a href={whatsappLink} target="_blank" rel="noreferrer" className="fixed bottom-5 right-5 bg-green-500 hover:bg-green-600 text-white p-4 rounded-full shadow-2xl z-50">
